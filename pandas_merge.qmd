...
# on argument is optional as long as the key name is same in both d-sets
```

```{python}
pd.merge(people, test_info_different_name, left_on='name', 
right_on='first_name')
```
...
```{python}
pd.merge(customer_details, order_records, left_on='id_number', 
...
```{python}
pd.merge(country_continents, tb_cases_children,  how='left', left_on='country.name.en', right_on='country')
```
